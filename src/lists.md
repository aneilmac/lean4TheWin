...
We've played around with `#eval`, `#check` and some simple types.
Before we move onto theorem solving, we're going to cement our
knowledge with a new type `List` and some _functions_ we can 
use to manipulate lists.
...
Lists are defined using between square brackets `[]`, 
with each element separated
...
Can you guess what happens when we append an empty list to a list?
...
We read this as 'a list of natural numbers'.
